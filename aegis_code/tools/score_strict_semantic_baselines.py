...
# Reviewer-safe conservative mapping: only explicit semantic overlap counts.
RULE_KEYWORDS = {
    "VU001": ["signature", "auth", "authentication", "recover", "ecdsa", "signer"],
    "VU002": ["nonce", "replay"],
    "VU003": [
        "return 0",
        "return success",
    ...
    "VU004": ["userophash", "hash", "message hash", "recompute"],
    "VU005": ["nonce update", "increment nonce", "consume nonce", "replay"],
    "VU007": [
        "entrypoint",
        "msg.sender",
        "caller",
...
def matches_rule(text: str, rule: str) -> bool:
...
def main():
    ...
    for row in gold_rows:
        ...
        for rule in RULES:
            ...
            pred = any(matches_rule(text, rule) for text in alert_texts)
            if pred and gt:
                per_rule_counts[rule]["tp"] += 1
                if len(aligned_examples) < 20:
    ...
    out = {
        "tool": args.tool,
        "granularity": "strict-function-type-reason",
        "matching_policy": "alert line must fall inside validateUserOp and alert text must explicitly overlap the target rule's semantic keyword family",
        "rules": rules_out,
